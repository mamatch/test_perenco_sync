...
- criticality (CMMS `PC`/`SCE` codes) maps to a governed MDM attribute via [`cmms_to_mdm.py`](pipeline/pipeline/cmms_to_mdm.py)'s [`CRITICALITY_TO_ATTRIBUTE`](pipeline/pipeline/cmms_to_mdm.py#L39), with an unrecognised code flagged as a data-quality issue rather than dropped silently ([`DECISIONS.md` #7](DECISIONS.md#L15)).
